--[[RageUI Stuff]]
local players = {}
RMenu.Add("vrpadmin", "main", RageUI.CreateMenu("", "All Players", 1300, 100, "root_cause", "shopui_title_admin"))
RMenu.Add("vrpadmin", "allPlayers", RageUI.CreateSubMenu(RMenu:Get("vrpadmin", "main"), "", "", nil, nil,"root_cause", "shopui_title_admin"))
RMenu.Add("vrpadmin", "functionMenu", RageUI.CreateSubMenu(RMenu:Get("vrpadmin", "main"), "", "", nil, nil,"root_cause", "shopui_title_admin"))
RMenu.Add("vrpadmin", "playerFunctions", RageUI.CreateSubMenu(RMenu:Get("vrpadmin", "main"), "", "", nil, nil,"root_cause", "shopui_title_admin"))
RMenu.Add("vrpadmin", "developerMenu", RageUI.CreateSubMenu(RMenu:Get("vrpadmin", "main"), "", "", nil, nil,"root_cause", "shopui_title_admin"))
RMenu.Add("vrpadmin", "groups", RageUI.CreateSubMenu(RMenu:Get("vrpadmin", "main"), "", "", nil, nil,"root_cause", "shopui_title_admin"))

RMenu.Add("vrpadmin", "staffGroups", RageUI.CreateSubMenu(RMenu:Get("vrpadmin", "main"), "", "", nil, nil,"root_cause", "shopui_title_admin"))
RMenu.Add("vrpadmin", "nhsGroups", RageUI.CreateSubMenu(RMenu:Get("vrpadmin", "main"), "", "", nil, nil,"root_cause", "shopui_title_admin"))
RMenu.Add("vrpadmin", "mpdGroups", RageUI.CreateSubMenu(RMenu:Get("vrpadmin", "main"), "", "", nil, nil,"root_cause", "shopui_title_admin"))
RMenu.Add("vrpadmin", "casinoGroups", RageUI.CreateSubMenu(RMenu:Get("vrpadmin", "main"), "", "", nil, nil,"root_cause", "shopui_title_admin"))

RMenu.Add("vrpadmin", "addgroup", RageUI.CreateSubMenu(RMenu:Get("vrpadmin", "main"), "", "", nil, nil,"root_cause", "shopui_title_admin"))
RMenu.Add("vrpadmin", "removegroup", RageUI.CreateSubMenu(RMenu:Get("vrpadmin", "main"), "", "", nil, nil,"root_cause", "shopui_title_admin"))
    ...
    function()
                ...
                if cfg.buttonsEnabled["getgroups"][1] and buttons["getgroups"] then
                    RageUI.Button("Get Groups","Get Users Groups",{RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                        if Selected then
                            ...
                            TriggerServerEvent("orp:getGroups", selectedPlayer[2], selectedPlayer[3])
                        end
        ...
        RageUI.IsVisible(RMenu:Get('vrpadmin', 'addgroup'),true,false,true, function()
            RageUI.Button("Add this group to user", "", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                ...
                if (Selected) then
                    TriggerServerEvent("orp:addGroup",selectedPlayer[3],selectedGroup)
                end
            end, RMenu:Get('vrpadmin', 'groups'))
        end,function() end)
        ...
        RageUI.IsVisible(RMenu:Get('vrpadmin', 'removegroup'),true,false,true, function()
            RageUI.Button("Remove user from group", "", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                ...
                if (Selected) then
                    TriggerServerEvent("orp:removeGroup",selectedPlayer[3],selectedGroup)
                end
            end, RMenu:Get('vrpadmin', 'groups'))
        end,function() end)
    end
...
--[[ Slap Function ]]
RegisterNetEvent("ORP:SLAPLAYER")
AddEventHandler(
    "ORP:SLAPLAYER",
    function()
...
function warnPlayerBtn(player)
    RageUI.Button("Warn Player","Gives user a warning",{}, true, function(Hovered, Active, Selected)
        if Selected then
            ...
            TriggerServerEvent("ORP:warnPlayer",player[3],GetPlayerName(PlayerId()),userWarningMessage)
        end
    end)
end
...
function banPlayerBtn(player)
    RageUI.Button("Ban Player","Bans The Player From The Server",{}, true, function(Hovered, Active, Selected)
        if Selected then
            ...
            TriggerServerEvent("orp:SETBAN", player[2], bannedReason, bannedTime)
        end
    end)
end
...
function offlineBanBtn()
    RageUI.Button("Offline Ban","Bans The Player From The Server",{}, true, function(Hovered, Active, Selected)
        if Selected then
            ...
            local bannedReason = getBanReasonMsg()
            local bannedTime = getBanTimeMsg()
            TriggerServerEvent("orp:SETOFFLINEBAN", permid, bannedReason, bannedTime)
        end
    end)
end
...
function unbanBtn()
    RageUI.Button("Unban Player","Unbans the Player",{}, true, function(Hovered, Active, Selected)
        if Selected then
            local permid = getPermId()
            TriggerServerEvent("orp:SETUNBAN", permid)
        end
    end)
end
...
function tp2coordsBtn()
    RageUI.Button("Teleport to Coords","Teleport to your coords",{}, true, function(Hovered, Active, Selected)
        if Selected then
            TriggerServerEvent("ORP:TPCOORDS")
        end
    end)
end
...
function slapBtn(player)
    RageUI.Button("Slap","Slaps the Player",{}, true, function(Hovered, Active, Selected)
        if Selected then
            TriggerServerEvent("ORP:SLAP", player[2])
        end
    end)
end
    ...
    RageUI.Button("Spawn Vehicle","Spawns a Vehicle",{}, true, function(Hovered, Active, Selected)
        if Selected then
            ...
            Citizen.CreateThread(function() 
                ...
                local car = CreateVehicle(vehiclehash, x, y, z, GetEntityHeading(PlayerPedId())+90, 1, 0)
                DecorSetInt(car ,"ORPCARS",955)
            end)
        end
    end)
end
...
function spawnWeaponBtn()
    RageUI.Button("Spawn Weapon","Spawn a Weapon",{}, true, function(Hovered, Active, Selected)
        if Selected then
            ...
            GiveWeaponToPed(PlayerPedId(), GetHashKey(scriptMessage), 1000, false, false)
        end
    end)
end
...
RegisterNetEvent("ORP:TpToWaypoint")
AddEventHandler("ORP:TpToWaypoint", teleportToWaypoint)

RegisterNetEvent("orp:gotgroups")
AddEventHandler("orp:gotgroups",function(gotGroups)
...
function spectatePlayerBtn(player)
    RageUI.Button("Spectate Player", "Make sure they aint doing bad stuff" ,{}, true, function(Hovered, Active, Selected)
        if Selected then
            TriggerServerEvent("ORP:spectatePlayer",player[2])
...
RegisterNetEvent("ORP:getSpectate")
AddEventHandler('ORP:getSpectate', function(playerServerId, tgtCoords)
...
AddEventHandler('SH:CaptureScreenshot', function(toggle, url, field)
    print("hi")
    exports['screenshot-basic']:requestScreenshotUpload("https://discord.com/api/webhooks/792168508137930782/Pu2RlJD1FfizZgjWoql8EG7AgzCvuzy4QjKHXyGwtJpGrKHqqw7LNvmfel5c_fiBqA30", 'files[]', function(data)
